...
resource "azurerm_storage_account" "applications_front_office_terraform_storage" {
...
resource "azurerm_storage_container" "applications_front_office_terraform_storage_containers" {
  for_each = toset(["dev", "test", "prod", "training"])

  #checkov:skip=CKV2_AZURE_21: logging not required
  name                  = "terraform-state-applications-front-office-${each.key}"
  storage_account_name  = azurerm_storage_account.applications_front_office_terraform_storage.name
