...
resource "azurerm_storage_account" "library_terraform_storage" {
...
resource "azurerm_storage_container" "library_terraform_storage_containers" {
  for_each = toset(["dev", "test", "prod"])

  #checkov:skip=CKV2_AZURE_21: logging not required
  name                  = "terraform-state-library-service-${each.key}"
  storage_account_id    = azurerm_storage_account.library_terraform_storage.id
