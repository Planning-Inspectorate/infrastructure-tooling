...
resource "azurerm_storage_account" "consultees_terraform_storage" {
...
resource "azurerm_storage_container" "consultees_terraform_storage_containers" {
  for_each = toset(["dev", "test", "prod", "training"])

  #checkov:skip=CKV2_AZURE_21: logging not required
  name                  = "terraform-state-consultees-${each.key}"
  storage_account_id    = azurerm_storage_account.consultees_terraform_storage.id
