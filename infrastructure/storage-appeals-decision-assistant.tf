resource "azurerm_storage_account" "appeals_decision_assistant_terraform_storage" {
...
resource "azurerm_storage_container" "appeals_decision_assistant_terraform_storage_containers" {
  for_each = toset(["dev", "test", "prod", "training"])

  #checkov:skip=CKV2_AZURE_21: logging not required
  name                  = "terraform-state-appeals-decision-assistant-${each.key}"
  storage_account_id    = azurerm_storage_account.appeals_decision_assistant_terraform_storage.id
